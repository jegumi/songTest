...
public class MainSearchFragment extends ListFragment {
    ...
    private void setSearchViewQuery(final String query) {
        if (mSearchView != null) {
            // Set the query text after all UI events are done on mSearchView else it doesn't work when called in onCreateOptionsMenu after rotation.
    ...
    private void setAdapter(ResultsPage resultPage) {
        ...
        if (parentActivity != null && resultPage != null && resultPage.results.artist != null && resultPage.results.artist.length > 0) {
